...
export async function registerRoutes(app: Express): Promise<Server> {
  ...
  // Select proposal winner and set up payment/escrow
  app.post('/api/proposals/:proposalId/select-winner', authenticate, async (req: any, res) => {
    try {
      if (req.user.role !== 'teacher') {
        return res.status(403).json({ message: 'Only teachers can select winners' });
      }
      
      const { proposalId } = req.params;
      const { budget, paymentType, schedules } = req.body;
      ...
      // Verify teacher has access to this proposal
      const assignment = await storage.getAssignment(proposal.assignmentId);
      if (!assignment || assignment.teacherId !== req.user.id) {
        return res.status(403).json({ message: "Access denied to this proposal" });
      }
      ...
      if (proposal.status !== 'approved') {
        return res.status(400).json({ message: "Only approved proposals can be selected as winners" });
      }
      
      // Select winner
      const updatedProposal = await storage.selectProposalWinner(
        proposalId, 
        req.user.id, 
        budget, 
        paymentType
      ...
      const escrowTransaction = await storage.createEscrowTransaction({
        ...
        amount: budget,
        status: 'in_escrow',
        description: `Escrow for proposal: ${proposal.title}`,
        createdBy: req.user.id
      });
      
      // Create payment schedules if split payment
      if (paymentType === 'split_payment' && schedules && schedules.length > 0) {
        await storage.createPaymentSchedulesForProposal(
          proposalId,
          budget,
      ...
      res.json({ 
        ...
        escrowTransaction,
        message: "Winner selected and escrow created successfully" 
      });
    } catch (error) {
  ...
  app.post('/api/escrow/:transactionId/release', authenticate, async (req: any, res) => {
    try {
      if (req.user.role !== 'teacher') {
        return res.status(403).json({ message: 'Only teachers can release escrow' });
      }
      
      const { transactionId } = req.params;
      const { notes } = req.body;
      
      // For now, we'll implement a simple approach to find the transaction
      // In a real implementation, you'd have a direct method to get transaction by ID
      try {
        // Release escrow 
        const releasedTransaction = await storage.approveEscrowRelease(
          transactionId,
          req.user.id,
          notes
        );
        ...
        return res.status(404).json({ message: "Escrow transaction not found or cannot be released" });
      }
    } catch (error) {
